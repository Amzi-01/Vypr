...
not usable from mingw, so it cannot be cross-compiled from Linux.
    ...
    ./gsync               push include/ and guest/ into C:\sash
    ./gssh 'C:\sash\build-agent.bat'    compile it there

`build-agent.bat` belongs in `C:\sash\` on the guest. It invokes `cl` directly
...
**Stop `sashd` and let the agent exit on its own.** Do not `taskkill` it: the
...
guest reboot clears it. This has cost two reboots.
    ...
    pkill -x sashd        # the agent exits within a few seconds
    ./gssh 'schtasks /end /tn sash-agent'    # if it is being stubborn
